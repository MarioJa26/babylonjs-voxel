export class Lod4Shader {
  ...
  `;
    ...
    void applyDitherFade() {
      if (abs(lodFadeDirection) < 0.5) return;
    ...
    vec3 applyTintBucket(vec3 color, float bucket) {
      ...
      vec3 tint = vec3(1.0);
      float sat = 1.0;
      if (b == 1.0) { tint = vec3(0.96, 0.98, 1.02); sat = 0.88; }
      else if (b == 2.0) { tint = vec3(1.04, 1.00, 0.92); sat = 0.90; }
      else if (b == 3.0) { tint = vec3(0.92, 1.06, 0.92); sat = 1.05; }
      else if (b == 4.0) { tint = vec3(0.90, 0.98, 1.08); sat = 0.90; }
      else if (b == 5.0) { tint = vec3(1.05, 0.97, 0.90); sat = 0.95; }
