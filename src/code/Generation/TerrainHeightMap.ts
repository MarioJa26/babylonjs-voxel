...
export class TerrainHeightMap {
	...
	static {
		TerrainHeightMap.params = GenerationParams;
		TerrainHeightMap.riverGenerator = new RiverGenerator(
			TerrainHeightMap.params,
		);
		const prng = Alea(TerrainHeightMap.params.SEED);

		TerrainHeightMap.temperatureNoise = createFastNoise2D({
			seed: Squirrel3.get(1, (prng() * 0xffffffff) | 0),
			fractalType: FractalType.None,
			frequency: GenerationParams.TEMPERATURE_NOISE_SCALE,
		});
		TerrainHeightMap.humidityNoise = createFastNoise2D({
			seed: Squirrel3.get(2, (prng() * 0xffffffff) | 0),
			fractalType: FractalType.None,
			frequency: GenerationParams.HUMIDITY_NOISE_SCALE,
		});
		TerrainHeightMap.continentalnessNoise = createFastNoise2D({
		...
		TerrainHeightMap.erosionSpline = new Spline([
			{ t: -1.0, v: 11.0 },
			{ t: -0.8, v: 0.8 },
			{ t: -0.5, v: 0.6 },
			{ t: 0.0, v: 0.4 },
		...
		TerrainHeightMap.peaksAndValleysSpline = new Spline([
			{ t: -1.0, v: -60 },
			{ t: -0.6, v: -25 },
			{ t: -0.2, v: -15 },
			{ t: 0.2, v: 15 },
	...
	public static getTerrainSample(
		...
		const key = TerrainHeightMap.encodeKey(worldX, worldZ);
		const cached = TerrainHeightMap.sampleCache.get(key);
		if (cached !== undefined) return cached;

		// --- Noise evaluation (each function called exactly once) ---
		const continentalness = TerrainHeightMap.continentalnessNoise(
			worldX,
			worldZ,
		);
		const temperature =
			(TerrainHeightMap.temperatureNoise(worldX, worldZ) + 1) / 2;
		const humidity = (TerrainHeightMap.humidityNoise(worldX, worldZ) + 1) / 2;
		const riverNoise = TerrainHeightMap.riverGenerator.getRiverNoise(
			worldX,
			worldZ,
		);
		...
		const baseHeight =
			...
			TerrainHeightMap.continentalnessSpline.getValue(continentalness);
		const detail = TerrainHeightMap.computeDetail(
			worldX,
			worldZ,
			baseHeight,
			riverAbs,
		...
		if (TerrainHeightMap.sampleCache.size > TerrainHeightMap.MAX_CACHE_SIZE) {
			TerrainHeightMap.sampleCache.clear();
		}
		TerrainHeightMap.sampleCache.set(key, sample);
		return sample;
	}
	...
	public static getBiome(x: number, z: number): Biome {
		return TerrainHeightMap.getTerrainSample(x, z).biome;
	}
	...
	public static getFinalTerrainHeight(worldX: number, worldZ: number): number {
		return TerrainHeightMap.getTerrainSample(worldX, worldZ).height;
	}
	...
	public static getCachedRiverNoise(worldX: number, worldZ: number): number {
		return TerrainHeightMap.getTerrainSample(worldX, worldZ).riverNoise;
	}
	...
	private static computeDetail(
		...
		const erosion = TerrainHeightMap.erosionNoise(x, z);
		const pv = TerrainHeightMap.peaksAndValleysNoise(x, z);
		...
		const roughness =
			TerrainHeightMap.erosionSpline.getValue(erosion) * effectiveRiverFactor;
		const detail =
			TerrainHeightMap.peaksAndValleysSpline.getValue(pv) * roughness;
		const riverDepth =
			TerrainHeightMap.riverGenerator.getRiverDepth(riverAbs) *
			surfaceRiverInfluence;
	...
	public static getOctaveNoise(x: number, z: number): number {
		return TerrainHeightMap.getTerrainSample(x, z).height; // already = floor(base + detail), so return directly
	}
}
