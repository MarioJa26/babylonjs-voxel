...
function singleSimplex3(seed: i32, x: f32, y: f32, z: f32): f32 {
  ...
  if (ax0 >= ay0 && ax0 >= az0) {
    ...
    if (b > 1) {
      ...
      const b2: f32 = b * b;
      value +=
        b2 * b2 * gradCoord3(seed, i - xNSign * PRIME_X, j, k, x0 + f32(xNSign), y0, z0);
    }
  } else if (ay0 >= az0) {
    let b: f32 = a + ay0 + ay0;
    if (b > 1) {
      ...
      const b2: f32 = b * b;
      value +=
        b2 * b2 * gradCoord3(seed, i, j - yNSign * PRIME_Y, k, x0, y0 + f32(yNSign), z0);
    }
  } else {
    let b: f32 = a + az0 + az0;
    if (b > 1) {
      ...
      const b2: f32 = b * b;
      value +=
        b2 * b2 * gradCoord3(seed, i, j, k - zNSign * PRIME_Z, x0, y0, z0 + f32(zNSign));
    }
  }
...
function singleSimplex2L4(seed: i32, x: v128, y: v128): v128 {
  ...
  let n0: v128 = f32x4.mul(
    f32x4.mul(a2, a2),
    gradL2(seed, iP, jP, x0, y0),
  );
  n0 = selectB(f32x4.gt(a, zero), n0, zero);

  // c corner
  const c: v128 = f32x4.add(f32x4.mul(f32x4.splat(C1), t), f32x4.add(f32x4.splat(C2), a));
  ...
  let n2: v128 = f32x4.mul(
    f32x4.mul(c2, c2),
    gradL2(seed, i32x4.add(iP, i32x4.splat(PRIME_X)), i32x4.add(jP, i32x4.splat(PRIME_Y)), x2, y2),
  );
  ...
  const b2: v128 = f32x4.mul(b, b);
  const gB: v128 = gradL2(seed, iP, i32x4.add(jP, i32x4.splat(PRIME_Y)), x1B, y1B);
  const gC: v128 = gradL2(seed, i32x4.add(iP, i32x4.splat(PRIME_X)), jP, x1C, y1C);
...
function fractal2L4(
  ...
  if (fractalType == 1) {
    ...
    for (let o: i32 = 0; o < octaves; o++) {
      ...
      amp = f32x4.mul(amp, f32x4.add(one, f32x4.mul(f32x4.sub(t, one), splatWs)));
      x = f32x4.mul(x, splatLac);
      y = f32x4.mul(y, splatLac);
      amp = f32x4.mul(amp, splatGain);
    }
  ...
  if (fractalType == 3) {
    ...
    for (let o: i32 = 0; o < octaves; o++) {
      let t: v128 = f32x4.mul(f32x4.add(singleSimplex2L4(seed++, x, y), one), splatPp);
      ...
      const n: v128 = selectB(f32x4.lt(t, one), t, f32x4.sub(f32x4.splat(2.0), t));
      sum = f32x4.add(
        sum,
        f32x4.mul(f32x4.mul(f32x4.sub(n, half), f32x4.splat(2.0)), amp),
      );
      amp = f32x4.mul(amp, f32x4.add(one, f32x4.mul(f32x4.sub(n, one), splatWs)));
      x = f32x4.mul(x, splatLac);
      y = f32x4.mul(y, splatLac);
      amp = f32x4.mul(amp, splatGain);
    }
...
function fractal3L4(
  ...
  if (fractalType == 1) {
    ...
    for (let o: i32 = 0; o < octaves; o++) {
      ...
      amp = f32x4.mul(
        amp,
        f32x4.add(one, f32x4.mul(f32x4.mul(f32x4.sub(n, one), f32x4.splat(0.5)), splatWs)),
      );
      x = f32x4.mul(x, splatLac);
      y = f32x4.mul(y, splatLac);
      z = f32x4.mul(z, splatLac);
  ...
  if (fractalType == 3) {
    ...
    const half: v128 = f32x4.splat(0.5);
    for (let o: i32 = 0; o < octaves; o++) {
      let t: v128 = f32x4.mul(f32x4.add(singleSimplex3L4(seed++, x, y, z), one), splatPp);
      const tt: v128 = f32x4.mul(t, f32x4.splat(0.5));
      t = f32x4.sub(
        t,
        f32x4.mul(
...
export function noise_scalar_3d(
  ...
  return fractal3(seed, x, y, z, fractalType, octaves, lacunarity, gain, ws, pp);
}
...
export function noise_fill_2d(
  ...
  for (let row: i32 = 0; row < height; row++) {
    ...
    for (; col < widthAligned; col += 4) {
      ...
      store<v128>(
        out + idx * 4,
        fractal2L4(seed, xs, ysT, fractalType, octaves, lacunarity, gain, ws, pp),
      );
      idx += 4;
    }
    for (; col < width; col++) {
...
export function noise_fill_3d(
  ...
  if (noiseType != 1) {
    for (let i: i32 = 0; i < width * height * depth; i++) store<f32>(out + i * 4, 0);
    return;
  }
  const widthAligned: i32 = width & ~3;
  let idx: i32 = 0;
    ...
    for (let slice: i32 = 0; slice < depth; slice++) {
      ...
      for (let row: i32 = 0; row < height; row++) {
        ...
        for (; col < widthAligned; col += 4) {
          ...
          store<v128>(
            out + idx * 4,
            fractal3L4(seed, xv, yv, zv, fractalType, octaves, lacunarity, gain, ws, pp),
          );
          idx += 4;
        }
        for (; col < width; col++) {
          ...
          store<f32>(
            out + idx * 4,
            fractal3(seed, x, yS, zS, fractalType, octaves, lacunarity, gain, ws, pp),
          );
          idx++;
        }
      }
...
export function surface_density_band(
  ...
  for (; i < aligned; i += 4) {
    ...
    const anyInside: i32 = i32x4.extract_lane(insideRange, 0) | i32x4.extract_lane(insideRange, 1) |
      i32x4.extract_lane(insideRange, 2) | i32x4.extract_lane(insideRange, 3);
    ...
    if (!anyInside) {
      ...
      const nBase0: f32 = noise_scalar_3d(
        ...
        baseNoiseZ,
        freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
      );
      ...
      const nOh0: f32 = noise_scalar_3d(
        f32x4.extract_lane(ohX, 0), f32x4.extract_lane(ohY, 0), f32x4.extract_lane(ohZ, 0),
        freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
      );
      const nOh1: f32 = noise_scalar_3d(
        f32x4.extract_lane(ohX, 1), f32x4.extract_lane(ohY, 1), f32x4.extract_lane(ohZ, 1),
        freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
      );
      const nOh2: f32 = noise_scalar_3d(
        f32x4.extract_lane(ohX, 2), f32x4.extract_lane(ohY, 2), f32x4.extract_lane(ohZ, 2),
        freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
      );
      const nOh3: f32 = noise_scalar_3d(
        f32x4.extract_lane(ohX, 3), f32x4.extract_lane(ohY, 3), f32x4.extract_lane(ohZ, 3),
        freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
      );
  ...
  for (; i < count; i++) {
    ...
    if (rel > influenceRange || rel < -influenceRange) {
      ...
      d =
        rel +
        noise_scalar_3d(
          baseNoiseX, y * yFreq, baseNoiseZ,
          freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
        ) * baseAmp +
        noise_scalar_3d(
          overhangBaseX + y * 0.0044, y * 0.012, overhangBaseZ - y * 0.0036,
          freq, noiseType, fractalType, transformType, seed, octaves, lacunarity, gain, ws, pp,
        ) * overhangAmp +
        cliffContribution;
    }
