...
function isValidHeldItemSelection(selection: HeldItemSelectionData): boolean {
	const itemId = selection.itemId;
	if (!Number.isInteger(itemId) || itemId < 0 || itemId > 65535) return false;
	if (itemId === HELD_ITEM_EMPTY) return selection.blockState === 0;
	if (!validHeldItemIds.has(itemId) && !isRegisteredBlockId(itemId))
		return false;
	return (
		Number.isInteger(selection.blockState) &&
		selection.blockState >= 0 &&
		selection.blockState <= HELD_ITEM_BLOCK_STATE_MAX
	);
}
...
export class VoxelRoom extends Room {
	...
	private readonly heldItemSelectionScratch: HeldItemSelectionData = {
		itemId: 0,
		blockState: 0,
	};
	...
	async onJoin(client: Client, options: { name?: string }) {
		...
		try {
			...
			for (const [sid, p] of this.players) {
				if (sid === client.sessionId || p.heldItemId === HELD_ITEM_EMPTY) {
					continue;
				}
				client.sendBytes(
					"binary",
					encodePlayerHeldItem({
						index: p.index,
						itemId: p.heldItemId,
						blockState: p.heldItemBlockState,
					}),
				);
			}
	...
	private tick(deltaMs: number): void {
		...
		for (let i = 0; i < mobEvents.length; i++) {
			...
			if (event.kind === ServerMobEventKind.Spawn) {
				this.broadcastBytes(
					"binary",
					encodeMobSpawn(
						event.mob.id,
			...
			} else if (event.kind === ServerMobEventKind.Impact) {
	...
	private decodeAndHandleBinaryMessage(client: Client, data: Uint8Array): void {
		...
		switch (msgType) {
			...
			case MessageType.HeldItemSelect: {
				if (dec.remaining !== 3)
					throw new Error("Invalid held item selection length");
				const selection = decodeHeldItemSelectInto(
					dec,
					this.heldItemSelectionScratch,
				);
				const player = this.players.get(client.sessionId);
				if (!player) return;
				if (!isValidHeldItemSelection(selection)) return;
				if (
				...
				this.broadcastBytes(
					...
					encodePlayerHeldItem({
						...
						blockState: selection.blockState,
					}),
					{},
				);
				break;
	...
	private handleContainerSetSlot(
		...
		if (!viewers?.has(client.sessionId)) {
			this.sendContainerRejected(
				client,
				x,
				y,
